...
func ProcessBlocks() error {
	...
	for {
	    fmt.Println("ProcessBlocksFOR()")
	    fmt.Printf("PREVBLOCK: %+v\n\n", previousBlock)
		block := previousBlock
...
func ProcessBlock(keyMR string) error {
	...
	for {
		block := previousBlock
		fmt.Println("Processing block %v\n", block.PartialHash)
