...
func SaveDBlock(b *DBlock) error {
	...
	//WE MIGHT NOT NEED THIS LINE/MAP
	//DBlocks[b.KeyMR] = b
...
func LoadDBlock(hash string) (*DBlock, error) {
	fmt.Printf("LoadDBlock::::: %+v\n\n", hash)
	block, ok := DBlocks[hash]
	if ok == true {
		return block, nil
	}
	fmt.Println("LoadDBlock2()")
    ...
    rows.Close()
	fmt.Println("LoadDBlock3()")
    if reflect.DeepEqual(newBlock, new(DBlock)) {
	    fmt.Println("LoadDBlock REFLECT EMPTY")
        //newBlock is empty (zero'd)
    	return nil, nil
    }
	fmt.Printf("newBlock (from LoadDBlock): %+v\n\n\n", newBlock)
...
func LoadBlock(hash string) (*Block, error) {
    ...
    fmt.Printf("LOAD BLOCK about to qry: %+v\n\n", hash)
    ...
    fmt.Printf("LOAD BLOCK DONE: %+v\n\n", newBlock)
	return newBlock, nil
}


func LoadBlockEntries(block *Block) error {
    fmt.Printf("LOAD BLOCK ENTRIES : %+v\n\n", block)
...
/*func LoadEntry(hash string) (*Entry, error) {
	...
	if entry.ChainID == AnchorBlockID {
		ar, err := ParseAnchorChainData(entry.Content.Decoded)
		if err != nil {
...
/*func LoadChainIDByName(name string) (string, error) {
	...
	if err != nil {
		fmt.Errorf("LoadChainIDByName - %v", err)
		return "", err
	}
	if entry2 != nil {
...
/*func LoadDataStatus() *DataStatusStruct {
	...
	DataStatus = ds
	fmt.Errorf("LoadDataStatus DS - %v, %v", ds, ds2)
	return ds
}*/
